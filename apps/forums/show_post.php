			...
			while($row = $result->fetch_assoc())
			{
				if($row['replying_to'] == 0)
				{
					...
					if(isset($_SESSION['username']))
					{
						...
						{
							echo "<a href='comment.php?title=".$title."&post_id=".$id."&edit_comment=".$row['comment']."'>Edit</a>";
							echo " ";
							echo "<a href='comment.php?title=".$title."&post_id=".$id."&delete_comment=".$row['comment']."'>Delete</a>";
						}
